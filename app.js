// ===== 設定 =====
// ←←← あなたの Web アプリ（GAS）URLに差し替えてください
...
let state = {
  clinic: null,          // "001"
  monthStr: null,        // "YYYY-MM"（nullなら今月）
  clinicName: "",
  dates: [],             // ["10/1(水)", ...]
  dateSet: new Set(),    // dates の高速照合用
  rooms: [],
  schedule: {},          // { room: { "M/D(曜)": {...} } }
  holidays: [],          // ["10/4", ...]
  minYM: null,
...
// ===== Util =====
function jpDow(dt) { return ["日","月","火","水","木","金","土"][ dt.getDay() ]; }
...
// GAS の dates と一致するキーを安全に生成
function safeKey(year, monthZeroBase, day) {
  const m = monthZeroBase + 1; // 1-12
  const try1 = `${m}/${day}(${jpDow(new Date(year, monthZeroBase, day))})`;
  if (state.dateSet.has(try1)) return try1;
  // 念のため曜日不一致を吸収（時差やロケール影響があっても日付一致で拾う）
  // "10/4(" で始まるものを探す
  const prefix = `${m}/${day}(`;
  for (const s of state.dates) { if (s.startsWith(prefix)) return s; }
  return try1; // 最後の保険
}
...
function renderCalendar(){
  ...
  const holidaySet = new Set((state.holidays||[]).map(h => h.split('(')[0])); // 例: "10/4"
  ...
  for (let w=0; w<numWeeks; w++){
    ...
    for(let d=0; d<7; d++){
      ...
      const key = safeKey(year, month, day);
    ...
    state.rooms.forEach((room, rIdx)=>{
      ...
      tdRoom.classList.add('week-band');
      ...
      for(let d=0; d<7; d++){
        ...
        if (e && (e.name || e.displayName)) {
          ...
          // 医師がいる日だがこの診療科にデータがない → ハイフン（非グレー）
          td.textContent = '−';
        }
        tr.appendChild(td);
      }
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  document.getElementById('prevMonth').onclick = ()=>{
    ...
    const next = new Date(y, m-2, 1); // 前月
    // 範囲ガード（minYM がある場合）
    if (state.minYM) {
      const min = new Date(...state.minYM.split('-').map((n,i)=> i===1 ? Number(n)-1 : Number(n)));
      if (next < min) return;
    }
    state.monthStr = yyyymm(next);
    fetchSchedule().catch(err => alert(err));
  };
  ...
  document.getElementById('nextMonth').onclick = ()=>{
    ...
    if (state.maxYM) {
      const max = new Date(...state.maxYM.split('-').map((n,i)=> i===1 ? Number(n)-1 : Number(n)));
      if (next > max) return;
    }
    state.monthStr = yyyymm(next);
    fetchSchedule().catch(err => alert(err));
